 ...
 *
 * The interceptor runs as a preload script in SDK subprocesses (Claude, Copilot).
...
export const DEBUG = INTERCEPTOR_LOGGING_ENABLED &&
  (process.argv.includes('--debug') || process.env.WEFT_DEBUG === '1');

/** Config file path for reading settings in the SDK subprocess */
export const CONFIG_FILE = join(homedir(), '.weft', 'config.json');

/** Session directory — set by env var (subprocess) or setSessionDir() (main process) */
let _sessionDir: string | null = process.env.WEFT_SESSION_DIR || null;

// ============================================================================
// LOGGING
// ============================================================================

export const LOG_DIR = join(homedir(), '.weft', 'logs');
...
function getErrorFilePath(): string {
  ...
  return join(homedir(), '.weft', 'api-error.json');
}
 ...
 * - SDK subprocess: WEFT_SESSION_DIR env var (set by main process before spawn)
