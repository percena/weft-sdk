 ...
 *
 * The `ErrorCode` union is owned by `@weft/core` so the wire
...
import type { ErrorCode } from '@weft/core/types';
import { getProviderMetadata } from './stubs.ts';
...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  ...
  model_no_tool_support: {
    ...
    message: 'The selected model does not support tool/function calling, which is required for Weft. Please choose a model with tool support (e.g., Claude, GPT-4, Gemini).',
    actions: [
  ...
  sdk_binary_missing: {
    ...
    message:
      ...
      'or a security tool removed it). Reinstalling the app typically fixes this.',
    actions: [
      { key: 'r', label: 'Retry', action: 'retry' },
    ],
    canRetry: true,
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for Weft. Please choose a different model with tool support in Settings.`,
        originalError: errorMessage,
        providerInfo,
      };
    }
