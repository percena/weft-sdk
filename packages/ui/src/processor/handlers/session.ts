...
export function handleComplete(
  ...
  // Clear isQueued from any user messages once the turn completes. OpenAI's steer
...
export function handleUserMessage(
  ...
  if (existingIndex >= 0) {
    ...
    // - 'accepted'   → isQueued = false (OpenAI steer path: agent has the message)
    //
 ...
 *
 * X-E(b): also mirror the pending request into `pendingPermissionRequests`
 * so a later `permission_resolved` event can clear it by requestId. The
 * Effect is still emitted (the onPermissionAllow callback flow is unchanged);
 * this field is the symmetric state mirror. Copy-on-write: a new array
 * reference so the reducer stays pure.
 */
export function handlePermissionRequest(
  state: SessionState,
  event: PermissionRequestEvent
): ProcessResult {
  const pending = state.pendingPermissionRequests ?? []
  return {
    state: {
      ...state,
      pendingPermissionRequests: [...pending, event.request],
    },
    effects: [{
...
/**
 * Handle permission_resolved (X-E(b)) - clear the matching pending permission
 * request from session state and surface the outcome as an info message so the
 * transcript keeps the "Permission granted/denied" line. The structured
 * `detail` rides on the event for UI consumers; the reducer only needs the
 * requestId to clear the pending entry. Non-matching requests are preserved
 * (concurrent overlapping requests). Safe fallback: if no pending list exists
 * (e.g. the request arrived before the session window), the filter is a no-op.
 */
export function handlePermissionResolved(
  ...
  // Clear the matching pending request (copy-on-write).
  const pending = state.pendingPermissionRequests ?? []
  const remaining = pending.filter(r => r.requestId !== event.requestId)

  // Surface the outcome as an info message, preserving the pre-X-E(b)
  // transcript line ("Permission granted: reason" / "Permission denied").
  const infoMessage: Message = {
    id: generateMessageId(),
    role: 'info',
    content: event.message,
