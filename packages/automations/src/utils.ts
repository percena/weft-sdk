...
/**
 * Build the base WEFT_* environment variables shared by both prompt and webhook actions.
...
function buildBaseEventEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  // Payload fields as WEFT_ vars (raw — callers apply sanitization if needed)
 ...
 * - Only injects WEFT_WH_* user-defined vars from process.env (webhook secrets)
 * - Includes WEFT_* system vars derived from the event payload
 *
 * Users set webhook secrets in their shell profile:
 *   export WEFT_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
 *   export WEFT_WH_DISCORD_TOKEN="abc123"
 *
 * Then reference them in automations.json:
 *   "url": "${WEFT_WH_SLACK_URL}"
 *   "headers": { "Authorization": "Bearer ${WEFT_WH_DISCORD_TOKEN}" }
 */
export function buildWebhookEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  // User-defined webhook secrets: only WEFT_WH_* from process.env
