...
import type { StoredAttachment, ContentBadge } from '@weft/core'
...
function InlineBadge({ badge }: { badge: ContentBadge }) {
  return (
    ...
    >
      {badge.iconDataUrl?.toLowerCase().startsWith('data:image/') ? (
        <img
...
function InlineFileBadge({
  ...
  // Strip .weft workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.weft\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
