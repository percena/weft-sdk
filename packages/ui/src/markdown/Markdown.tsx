...
import { rehypeSanitizeInlineStyle } from './style-sanitizer.ts'
...
/**
 * Sanitize schema for agent-rendered markdown.
 *
 * Agent output is model/tool-controlled and rendered in the embedding host's
 * origin, so raw HTML parsed by `rehypeRaw` MUST be sanitized before it
 * reaches the DOM. This schema extends `rehype-sanitize`'s GitHub-style
 * `defaultSchema` (which already blocks `iframe`/`script`/`form`/`style`
 * elements, `object`/`embed`/`link`/`meta`, and all `on*` event-handler
 * attributes, and restricts `a[href]`/`img[src]` to safe URL schemes) with
 * what KaTeX + GFM need:
 *
 * - KaTeX MathML elements (`math`/`mrow`/`mi`/`mo`/`mn`/`msup`/`msub`/
 *   `mfrac`/`msqrt`/`mtext`/`mspace`/`mtable`/`mtr`/`mtd`/`msemantics`/
 *   `mstyle`/`annotation`) so `rehypeKatex` output survives sanitization.
 * - `className` globally — KaTeX CSS targets `.katex`/`.katex-mathml`/
 *   `.strut`/etc., and GFM task-list classes (`task-list-item`,
 *   `contains-task-list`) are applied via `className`.
 * - `style` (KaTeX struts set per-element heights via inline `style`).
 *   The attribute is allowed because KaTeX struts depend on it, but the value
 *   is filtered by `rehypeSanitizeInlineStyle` (see `style-sanitizer.ts`) to a
 *   tight layout allowlist (height, vertical-align, text-align, margin,
 *   padding, line-height) with any `url(…)` blocked — so CSS-driven data
 *   exfiltration (`background-image:url(attacker)`) and UI redressing
 *   (`position:fixed;…;opacity:0`) from untrusted model output are blocked.
 *   Residual risk: `className` is allowed globally (KaTeX/GFM classes need
 *   it); a host app with exploitable class names (e.g. Tailwind) could still be
 *   clickjacked via model-injected `class="fixed inset-0 opacity-0"` — keep the
 * tool allowlist minimal and assume prompt injection.
 * - `role` + `ariaHidden`/`ariaLabel`/`ariaDescribedBy`/`ariaLabelledBy`
 *   (KaTeX uses `aria-hidden` on the HTML mirror).
 * - `'data*'` wildcard — preserves `data-section-id`/`data-heading-level`
 *   from `remarkCollapsibleSections` and GFM footnote data attributes.
 * - `encoding` on `<annotation>` (KaTeX TeX source annotation).
 *
 * The GFM task-list `input` (checkbox, `checked`/`disabled`) is already
 * allowed by `defaultSchema` (with `required: {input: {disabled: true,
 * type: 'checkbox'}}`), so remark-gfm task lists survive unchanged.
 */
const sanitizeSchema: SanitizeSchema = {
  ...
  tagNames: [
    ...
    'mtext', 'mspace', 'mtable', 'mtr', 'mtd', 'msemantics', 'mstyle',
    'annotation',
  ],
  attributes: {
    ...
    '*': [
      ...
      'ariaDescribedBy',
      'ariaLabelledBy',
      // Wildcard: allows any `data-*` property (data-section-id,
      // data-heading-level, GFM footnote data attrs, etc.)
      'data*',
    ],
    annotation: ['encoding'],
  },
    ...
    return {
      ...
      code: ({ className, children, ...props }) => {
        ...
        if (match || isBlock) {
          ...
          // HTML preview blocks → render the HTML SOURCE as text in a <pre>
          // (the source is displayed, NOT executed/sandboxed). rehype-sanitize
          // also strips any raw HTML before it reaches here.
  ...
  return (
    <div className={cn('markdown-content', className)}>
      <ReactMarkdown
        ...
        rehypePlugins={[rehypeKatex, rehypeRaw, [rehypeSanitize, sanitizeSchema], rehypeSanitizeInlineStyle]}
