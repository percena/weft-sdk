...
8. **Provision + run** — start via your entrypoint (or `node run.mjs` after adapting imports); `assertWeftdCreds` fails fast on missing creds; `ensureApp` provisions on weftd. Templates host-seal `permission_mode: auto` at createSession; set the chat panel to `Auto` for unattended/e2e (both required).
...
- **`Auto` permission mode is dev/e2e ONLY** → host-sealing `config.permission_mode: "auto"` at createSession (demo templates) + panel `Auto` lets runs skip per-tool approval prompts under weftd fenced autonomy. Production MUST seal `ask`/`explore` (or omit → ask) with a real policy hook — AND verify the hook actually fires (write a contract test that a deny hook blocks a tool for every provider you use).
...
- **`Weft HTTP 409` / `llm_connection_unusable`** → the tenant's LLM connection (BYOK key / subscription) exists but is unusable — it must be fixed in the Weft console (re-connect the subscription or replace the key). Terminal until the user acts: retrying the run keeps failing with the same 409. Match the message substring `llm_connection_unusable` (or the human detail after `Weft HTTP 409:`) — that works on every published SDK. On SDK builds that export `WeftHttpError` from `@percena/weft/providers/flitro`, prefer `error instanceof WeftHttpError && error.code === 'llm_connection_unusable'` (or inspect `turn_failed.error.code` on the timeline) instead of parsing the message.
- **`Weft HTTP 402` / `quota_exceeded`** → the tenant's usage quota for the current billing window is exhausted (free-tier hard-stop is ON by default, so every free integrator WILL hit this). **Terminal for the window: do not retry** — every retry 402s until the window resets or the plan is upgraded. Correct handling: branch on `error instanceof WeftHttpError && error.code === 'quota_exceeded'` (or `turn_failed.error.code` via `readTurnFailedError` for queued sends) and surface an upgrade/limits UI ("you've used your included quota — upgrade or wait for the reset"), not a generic error toast or a retry button.
- **`Weft HTTP 403` / `identity_binding_required`** → a free-tier session is missing its verified identity binding (server-side fail-closed check). Not retryable as-is: route the user through your app's (re-)authentication path so the session is minted with a bound identity, then let them resend. Same branching pattern as above (`error.code === 'identity_binding_required'`).
...
- **Chat prompts on every write tool** → two layers: (1) the panel defaults to permission mode `ask` — switch the composer to `Auto` for unattended/e2e; (2) **host must seal `config.permission_mode: "auto"` on `POST /v1/sessions`** (templates already do this via the developer API key). weftd fenced autonomy refuses untrusted embed JWT elevate `ask`→`auto`; panel Auto alone is **not** enough if the session was sealed `ask`. Production should seal `ask` (or omit → default ask) + a real policy hook instead of host-sealing auto.
