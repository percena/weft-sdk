import { existsSync, readFileSync, statSync, writeFileSync, unlinkSync, renameSync } from 'node:fs';
import { extname, basename, resolve, join, relative } from 'node:path';
...
function readClipboardMacOS(): FileAttachment[] {
  ...
  try {
    const scriptFile = join(tmpdir(), `weft-clipboard-files-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
...
function readClipboardImageDataWindows(): FileAttachment | null {
  const tempFile = join(tmpdir(), `weft-clipboard-${Date.now()}.png`);
...
function readClipboardImageDataMacOS(): FileAttachment | null {
  ...
  try {
    const scriptFile = join(tmpdir(), `weft-clipboard-script-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
ObjC.import('Foundation');
