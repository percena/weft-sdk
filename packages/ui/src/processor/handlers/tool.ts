...
import type { SessionState, ToolStartEvent, ToolDeltaEvent, ToolResultEvent, TaskBackgroundedEvent, ShellBackgroundedEvent, TaskProgressEvent, TaskCompletedEvent } from '../types'
import type { Message } from '@weft/core'
import { isParentTaskTool } from '../utils/toolNames'
import {
  findToolMessage,
  updateMessageAt,
  appendMessage,
...
export function handleToolStart(
  ...
  if (existingIndex !== -1) {
    // Two tool_start events arrive per call, and providers disagree on which
    // one carries the metadata: Claude sends empty input first then the full
    // input; Flitro sends the full {input, intent} first then a bare "running"
    // event. Coalesce by field — only overwrite when the new event actually
    // has a value — so the later event never wipes input/intent/display info.
    const existingMessage = session.messages[existingIndex]
    const updatedSession = updateMessageAt(session, existingIndex, {
      toolName: event.toolName ?? existingMessage?.toolName,
      toolInput: event.toolInput ?? existingMessage?.toolInput,
      toolIntent: event.toolIntent ?? existingMessage?.toolIntent,
      toolDisplayName: event.toolDisplayName ?? existingMessage?.toolDisplayName,
      toolDisplayMeta: event.toolDisplayMeta ?? existingMessage?.toolDisplayMeta,
      toolStatus: existingMessage?.toolStatus ?? 'executing',
      turnId: event.turnId ?? existingMessage?.turnId,
      parentToolUseId: event.parentToolUseId ?? existingMessage?.parentToolUseId,
    })
...
/**
 * Handle tool_delta - append live output while the tool is still executing.
 */
export function handleToolDelta(
  state: SessionState,
  ...
  if (toolIndex !== -1) {
    const currentMessage = session.messages[toolIndex]
    const currentOutput = currentMessage?.toolResult || currentMessage?.content || ''
    const updatedSession = updateMessageAt(session, toolIndex, {
      toolResult: currentOutput + event.delta,
      toolStatus: currentMessage?.toolStatus ?? 'executing',
      turnId: event.turnId ?? currentMessage?.turnId,
...
export function handleToolResult(
  ...
  if (toolIndex !== -1) {
    ...
    const completedResult = event.result || existingMessage?.toolResult || existingMessage?.content || ''
