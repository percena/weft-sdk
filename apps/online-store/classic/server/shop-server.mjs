// Online Store (classic) backend: REST CRUD + order state machine + SSE event
// feed. No weftd, no AI — the agent layer is added by the integrate-weft-kit
// skill on top of this. Run: node server/shop-server.mjs (SHOP_PORT to override).
...
// time, so the allowlist is actually applied (a module-scope const would be
// captured before .env loads → silently empty → any-origin reflection).
...
export function createShopServer({ state = createShopState() } = {}) {
  ...
  // Guarded SSE fan-out: iterate a snapshot of clients, skip+drop any
  // destroyed/ended, swallow per-write errors so one dead socket can't kill
  // the broadcast.
  state.onEvent((event) => {
    ...
    for (const res of [...sseClients]) {
      ...
      // Per-customer scoping: a connection only gets its own customer-scoped
      // events (cart/order); global events (product.updated, shop.reset) have
      // no customer_id → sent to everyone. Prevents cross-user SSE leaks.
  ...
  const server = createServer(async (req, res) => {
    ...
    // X-Weft-Actor is stamped by the Weft browser runtime for client-side tool
...
if (isMain) {
  ...
  server.listen(port, "127.0.0.1", () => {
    console.log(JSON.stringify({ ok: true, service: "online-store-classic", url: `http://127.0.0.1:${port}` }));
  });
}
