...
export interface AutomationsConfigDoctorReport extends ValidationResult {
  domain: 'automations';
  summary: {
    matcherCount: number;
    actionCount: number;
...
function runMatcherSemanticValidations(
  ...
  for (const [event, matchers] of Object.entries(config.automations)) {
    ...
    for (let i = 0; i < matchers.length; i++) {
      ...
      if (!matcher) continue;
      // Reject auto permission mode — automations must never bypass security checks.
      // The schema already excludes 'auto'; this is defense in depth for configs
      // validated through the object-based path.
      if (matcher.permissionMode === 'auto') {
        errors.push({
          file,
          path: `automations.${event}[${i}].permissionMode`,
          message: 'permissionMode "auto" bypasses all security checks and is not allowed for automations',
          severity: 'error',
          suggestion: 'Use "explore" or "ask" permission mode instead',
        });
      }
...
export function validateAutomationsContent(jsonString: string, fileName?: string): ValidationResult {
  ...
  const matcherCount = Object.values(config.automations as Record<string, unknown[]>).reduce(
...
export function createAutomationsConfigDoctorReport(
  jsonString: string,
  fileName?: string,
): AutomationsConfigDoctorReport {
...
function summarizeAutomationsConfig(jsonString: string): AutomationsConfigDoctorReport['summary'] {
  try {
    const parsed = JSON.parse(jsonString) as { automations?: Record<string, unknown> };
    let matcherCount = 0;
    let actionCount = 0;
    for (const matchers of Object.values(parsed.automations ?? {})) {
      ...
      actionCount += matchers.reduce((sum, matcher) => {
        const actions = matcher && typeof matcher === 'object'
          ? (matcher as { actions?: unknown }).actions
          : undefined;
