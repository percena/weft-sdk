...
The session-cookie name (``itsm_session``) + the SSE event name
(``itsm.event``, in the classic's events router) are the ITSM values; for
a new app the skill substitutes them.
"""
...
# /v1 call (itsm_session is not hop-by-hop); the allowlist makes that impossible
# by construction.
_PROXY_HEADER_ALLOW = {
...
def create_session_router(provisioning, *, toolset: str, app_name: str,
                          sessions: dict, session_cookie: str = "itsm_session") -> APIRouter:
    ...
    weftd_host = _u.host
    shop_public_base = os.environ.get("ITSM_PUBLIC_BASE", "").rstrip("/")
    ...
    async def create_session(request: Request):
        ...
        # the 'guest' fallback is demo-only; ship a 401 default so an integrator
        # who copies the template inherits a safe default instead of minting
        # weftd sessions for anyone with no cookie.
        ...
        try:
            ...
            session = await run_in_threadpool(
                provisioning.weftd_api, "POST", "/v1/sessions",
                {
                    ...
                    "app_id": ctx["app_id"],
                    "external_user_id": eu,
                    "external_account_id": body.get("external_account_id") or "demo-org",
                    "toolset": toolset,
                    "title": f"{app_name} — {eu}",
    ...
    async def proxy_v1(rest: str, request: Request):
        ...
        # idle stream); POST/PUT/DELETE are short JSON → read=30 catches a
        # mid-stream/headers stall. connect/pool/write are bounded for all.
        ...
        resp = None
        # Up to 3 retries. Pre-handshake errors (ConnectError/ConnectTimeout/
        # PoolTimeout) fire BEFORE any body bytes are sent → safe for ALL methods.
        # A ReadTimeout fires AFTER the body was sent (weftd MAY have received it
        # + created the run) → retry ONLY idempotent methods; for POST/PUT/DELETE
        # fail-closed (504) so the caller surfaces the error rather than
        # double-issuing createRun.
        pre_handshake = (httpx.ConnectError, httpx.ConnectTimeout, httpx.PoolTimeout)
        for attempt in range(4):
            try:
                resp = await client.send(req, stream=True)
                break
            except pre_handshake as e:
                if attempt < 3:
                ...
                if attempt < 3 and is_idempotent:
                    await asyncio.sleep(0.25 * (attempt + 1))
                    continue
                await client.aclose()
                return JSONResponse({"error": f"weftd proxy failed: {e}"}, status_code=504)
