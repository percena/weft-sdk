// weftd chat-session backend + /v1 reverse proxy (skill template).
//
// Provides CORS/OPTIONS + X-Weft-Actor stamping, POST /api/chat/session +
// :id/token, the guarded SSE fan-out helper, and proxyToWeftd (TLS-handshake
// retry, headersSent gating, pre-handshake-only transient-error retry).
// Upstream request headers are built via proxyHeaders(...) (an allowlist) so
// Cookie + hop-by-hop headers are never forwarded to weftd.
//
// writeJSON is a hoisted module-scope function (NOT an in-factory const) so
// wireSessionRoutes + proxyToWeftd can call it without TDZ hazards.
//
// Interface:
//   wireSessionRoutes(server, { weftdBase, apiKey, tenantId, ensureApp, weftdAPI,
//                               state, sseClients, sessions })
// Wraps the server's existing 'request' listener with a single-writer wrapper:
// CORS/OPTIONS + X-Weft-Actor + /api/chat/* + /v1/* are handled here; everything
// else delegates to the original listener. If `state` is provided, its onEvent
// is re-wired to the guarded SSE fan-out. sseClients/sessions are shared with
// the host app's own /api/events + cookie-session machinery.
...
// authenticated session cookie ONLY. The session map (cookie sid →
...
//
// `sessionCookie` is the host app's cookie name — substitute {{sessionCookie}}
// (or pass it into wireSessionRoutes). Never hardcode a product-specific name.
export function cid(req, sessions, sessionCookie = '{{sessionCookie}}') {
  const name = String(sessionCookie || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  if (!name) return 'guest'
  const sid = (req.headers.cookie || '').match(new RegExp(`(?:^|;\\s*)${name}=([^;]+)`))?.[1]
...
/**
 * Wire the weftd chat-session backend + /v1 reverse proxy onto the host app's
 * HTTP server. Wraps the server's existing request listener with a single-writer
 * that handles CORS/OPTIONS, X-Weft-Actor, /api/chat/session,
 * /api/chat/session/:id/token, and /v1/*; all other paths delegate to the
 * original (the host app's REST + static).
 *
 ...
 * @param {object} [opts.state]       - App state; if present, onEvent is re-wired to the guarded fan-out.
 * @param {Set} [opts.sseClients]     - Shared SSE client set (the host app's /api/events adds here).
 * @param {Map} [opts.sessions]       - Shared cookie-session map (for end_user_id resolution).
 * @param {string} [opts.sessionCookie] - Host session cookie name (default: {{sessionCookie}}).
 * @param {string} [opts.sseEventName]  - SSE event type string (default: {{sseEventName}}).
 */
export function wireSessionRoutes(server, {
  weftdBase, apiKey, tenantId, ensureApp, weftdAPI, state, sseClients, sessions,
  ...
  const _sseEventName = sseEventName
  ...
  const appPublicBase = (process.env.APP_PUBLIC_BASE || process.env.PUBLIC_BASE || '')
    .replace(/\/$/, '')
  ...
  // forwarded one); override with APP_PUBLIC_BASE when behind a fixed URL.
  const publicBaseURL = (req) => {
    if (appPublicBase) return appPublicBase
    const proto = String(req.headers['x-forwarded-proto'] ?? '').split(',')[0].trim() || 'http'
    const host = req.headers['x-forwarded-host'] ?? req.headers.host
    return host ? `${proto}://${host}` : ''
  }

  // Per-end-user SSE fan-out:
  ...
  // scopes each connection to its own end-user's events when the payload
  // carries end_user_id / user_id / customer_id; events without a scope go to
  // everyone. NOTE: state.onEvent APPENDS (Set-based, not replaces), so when
  // weftd is configured this runs ALONGSIDE the host app's own fan-out — both
  // fire (a duplicated write to each matching client; no leak, no miss).
  // SSE clients may set `res.weftEndUserId` (or legacy `res.weftCustomerId`)
  // when the connection is opened.
  const broadcast = (event) => {
    const line = `event: ${_sseEventName}\ndata: ${JSON.stringify(event)}\n\n`
    const scopeId = event?.data?.end_user_id
      ?? event?.data?.user_id
      ?? event?.data?.customer_id
      ?? event?.data?.order?.customer_id
      ?? null
    for (const res of [..._sseClients]) {
      if (res.destroyed || res.writableEnded) { _sseClients.delete(res); continue }
      const clientScope = res.weftEndUserId ?? res.weftCustomerId
      if (scopeId && clientScope && clientScope !== scopeId) continue
  ...
  // Reason: weftd is fronted by an edge () that negotiates HTTP/2 via
  // ALPN. Node's `http.request`/`https.request` is HTTP/1.1-only — it either
  // fails to parse the h2 frames the edge returns ("Expected HTTP/") or hangs
  // when limited to `http/1.1`. undici `fetch` speaks the edge's negotiated
  // protocol correctly AND streams `response.body` as a real ReadableStream,
  // which is what makes the long-lived timeline SSE actually stream (the prior
  // http.request proxy hung on the SSE handshake → the chat showed "Not
  // connected" and the SDK fell back to polling). Semantics preserved: buffer
  // the request body so a pre-handshake failure can retry without replaying
  // non-idempotent POSTs; retry ONCE on a connection-establishment error only
  // (before any response headers are written); once headers are sent (SSE/
  // streamed responses), mid-stream errors tear down the downstream response
  // instead of retrying. headersSent still gates the retry. After writeHead,
  // `res.flushHeaders()` is MANDATORY — Node otherwise buffers headers until
  // the first res.write()/end(), so an idle SSE stream (no body bytes until the
  // first event) would never reach the browser → "Not connected".
  const proxyToWeftd = async (req, res, url) => {
    // Buffer the request body so we can retry the upstream call on a
    // connection-ESTABLISHMENT error (the edge occasionally closes a connection
    // during the TLS handshake, notably concurrent with the long-lived timeline
    // SSE, surfacing as a fetch `TypeError: fetch failed` with an ECONNRESET/TLS
    // cause). The retry is narrowed to pre-handshake/connection errors ONLY:
    // these fire before any body bytes are sent, so retrying is safe even for
    // non-idempotent POSTs (createRun, permission-response, tool-outputs,
    // token). Mid-stream errors (after the response has started) can fire AFTER
    // weftd already received the body, so retrying would replay non-idempotent
    // requests and create duplicates — those are surfaced, not retried.
    // headersSent gates the error path for streamed/SSE responses, which have
    // already sent headers by the time any mid-stream error fires.
    const chunks = [];
    let proxyLen = 0;
  ...
  // Wrap the server's existing request listener (the host app's REST + static)
  ...
  server.on('request', async (req, res) => {
    ...
    // static, SSE) carries the allow-list.
    ...
    const m = req.method
    const c = cid(req, _sessions, _sessionCookie)
    ...
    try {
      ...
      // toolset "{{toolset}}" matches the named toolset provisioned by createProvisioning.
      if (p === '/api/chat/session' && m === 'POST') {
        ...
        // arbitrary end-user. (external_account_id is an org-scoping hint, not an
        // identity claim — keep your own auth on it if account spoofing is in
        // your threat model.)
        const endUserId = c ?? 'guest'
        // SECURITY: reject unauthenticated callers — ship a 401 default so an
        // integrator who copies the template inherits a safe default instead of
        // minting weftd sessions for anyone with no cookie.
        if (c === 'guest') {
        ...
        try {
          ...
          const session = await weftdAPI('POST', '/v1/sessions', {
            ...
            external_account_id: body.external_account_id ?? 'default-org',
            // The {{toolset}} toolset (named {{toolset}}_* tools, execution:client)
            // is resolved by flitro from the session's toolset name.
            toolset: '{{toolset}}',
            title: `{{appName}} — ${endUserId}`,
            credential: { token: `{{toolset}}-cred-${endUserId}-${Date.now()}`, scheme: 'bearer' },
      ...
      if (tokenMatch && m === 'POST') {
        ...
        try {
          ...
          const session = await weftdAPI('POST', `/v1/sessions/${encodeURIComponent(sessionId)}/token`, {
            credential: { token: `{{toolset}}-cred-${refreshUser}-${Date.now()}`, scheme: 'bearer' },
          })
    ...
    // Fall through to the host app's original handler (REST CRUD + static + SSE).
