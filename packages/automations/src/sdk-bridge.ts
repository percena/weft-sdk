...
export function buildEnvFromSdkInput(event: AgentEvent, input: SdkAutomationInput): Record<string, string> {
  ...
  switch (event) {
    case 'PreToolUse':
    case 'PostToolUse':
      if (input.tool_name) env.WEFT_TOOL_NAME = input.tool_name; // Tool names are internal, not user input
    ...
    case 'SessionStart':
      if (input.source) env.WEFT_SOURCE = input.source; // Internal values
      if (input.model) env.WEFT_MODEL = input.model;
      break;

    case 'SubagentStart':
    case 'SubagentStop':
      if (input.agent_id) env.WEFT_AGENT_ID = input.agent_id; // Internal values
