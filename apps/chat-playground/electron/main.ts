 ...
 *
 * Delegates to `@percena/weft-node/runtime` `listProviderModels`, which reads
 * local Claude (~/.claude/settings.json env) / Codex (~/.codex/config.toml)
 * config + probes the compatible gateway's /v1/models. The implementation
 * lives in the SDK (not the demo) so any consumer gets it; the same
 * ProviderModelDiscovery wire contract is produced server-side for the browser
 * (@percena/weft) path. The renderer's picker-default / auto-heal logic
 * (App.tsx) consumes the returned shape unchanged.
 */
