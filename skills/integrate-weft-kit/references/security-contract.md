...
- **Verified policy hook** — pass a `policy.hook` to `createHostAgentRuntime` for `ask`/`explore` mode; **write a contract test** that a deny hook blocks a tool for every provider you use. `Auto`/`execute` mode is **dev/e2e ONLY** — it auto-allows every tool call (a prompt-injected agent runs destructive/host tools with zero confirmation). Production MUST use `ask`/`explore` + the hook.
