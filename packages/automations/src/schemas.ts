...
import { z } from 'zod';
import { parsePermissionMode, normalizeThinkingLevel } from '@weft/core';
...
const ThinkingLevelInputSchema = z
  .enum(['off', 'low', 'medium', 'high', 'xhigh', 'max'] as [string, ...string[]])
  .transform((value: string) => normalizeThinkingLevel(value))
  .optional();
...
}).superRefine((data: { value?: unknown; from?: unknown; to?: unknown; contains?: string; not_value?: unknown }, ctx: z.RefinementCtx) => {
...
export const AutomationMatcherSchema = z.object({
  ...
  timezone: z.string().optional(),
  // 'auto' is deliberately excluded: automation prompts run unattended, and
  // permissionMode "auto" would bypass all security checks (see security review C2)
  permissionMode: z.preprocess(
    (val: unknown) => (typeof val === 'string' ? (parsePermissionMode(val) ?? val) : val),
    z.enum(['explore', 'ask'], {
      errorMap: () => ({ message: 'Invalid permissionMode — automations only allow "explore" or "ask" ("auto" is not permitted)' }),
    }).optional(),
  ),
 ...
 * Validators surface warnings for old names; schema parsing itself stays side-effect free.
 */
...
}).transform((data: { version?: number; automations?: Record<string, z.infer<typeof AutomationMatcherSchema>[]> }) => {
  const automations = data.automations ?? {};

  // Filter out invalid event names and rewrite deprecated aliases. Diagnostics
  // belong in validation/doctor results, not in schema transform side effects.
