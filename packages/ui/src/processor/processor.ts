...
import type { SessionState, ChatEvent, ProcessResult } from './types'
import { handleTextDelta, handleTextComplete } from './handlers/text'
import { handleToolStart, handleToolDelta, handleToolResult, handleTaskBackgrounded, handleShellBackgrounded, handleTaskProgress, handleTaskCompleted } from './handlers/tool'
import {
  handleComplete,
  handleError,
  handleTypedError,
...
export function processEvent(
  ...
  switch (event.type) {
    ...
    case 'tool_delta': {
      const newState = handleToolDelta(state, event)
      return { state: newState, effects: [] }
    }
    ...
    case 'permission_resolved':
      return handlePermissionResolved(state, event)
