...
import type { LoggedAutomationEvent, } from '../event-logger.ts';
...
export class EventLogHandler implements AutomationHandler {
  ...
  constructor(options: EventLogHandlerOptions) {
    ...
    if (options.onEventLost) {
      // Bridge the signature mismatch: logger calls (event: LoggedAutomationEvent) => void,
      // but the handler options expect (events: string[], error: Error) => void.
      // The stub logger never invokes onEventLost, so this wrapper satisfies the type check.
      this.logger.onEventLost = (_loggedEvent: LoggedAutomationEvent) => {
        // Cannot meaningfully convert a single LoggedAutomationEvent to (events[], Error)
        // in the stub — real implementation would bridge this properly.
      };
    }
  }
