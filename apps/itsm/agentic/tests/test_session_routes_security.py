...
[the Python `..` bypass] + streaming body cap). (§5 SSE cap is demo-level — in
the app's events router, not this template — so it is NOT tested here; §6 CORS
is app-level — the app's CORSMiddleware in main.py — also not here. The
integrator's SKILL.md §6 checklist covers both.)
...
try:  # app package context (integrator's app)
...
def _build_app(sessions: dict, session_cookie: str = "shop_session") -> FastAPI:
    app = FastAPI()
...
def _cookie(sessions: dict, user: str, session_cookie: str = "shop_session") -> str:
    sid = f"sess-{user}"
    sessions[sid] = user
...
def test_end_user_id_from_cookie_only():
    ...
    assert end_user_id(_Req({"shop_session": "sess-alice"}), sessions, "shop_session") == "alice"
    assert end_user_id(_Req({}), sessions, "shop_session") == "guest"               # no cookie
    assert end_user_id(_Req({"shop_session": "unknown"}), sessions, "shop_session") == "guest"  # invalid
    assert end_user_id(_Req({"other": "x"}), sessions, "shop_session") == "guest"    # wrong name
