...
import { executeWithRetry, redactUrl, isTransientFailure, createWebhookHistoryEntry, nodeDnsResolveAll } from '../webhook-utils.ts';
...
export class WebhookHandler implements AutomationHandler {
  ...
  private async handleEvent(event: AutomationEvent, payload: BaseEventPayload): Promise<void> {
    ...
    if (toExecute.length > 0) {
      const webhookOpts = { env, retry: { maxAttempts: 2 }, resolveIps: nodeDnsResolveAll };
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (!result.success) {
        // SECURITY (audit B6): redact the webhook URL before logging — webhook
        // URLs may embed a secret in the path (Slack/Discord inbound tokens).
        log.debug(`[WebhookHandler] ${redactUrl(result.url)} → ${result.error}`);
      }

      // Write history entry for each webhook execution.
      // Await for durability, but keep failures non-fatal.
      // SECURITY (audit B1): redact the URL persisted to the history file too.
      ...
      // SECURITY (audit B1): enqueue the UNEXPANDED action so no secret values
      // (auth tokens, the secret URL path) are persisted to the retry-queue
      // file. Capture only the non-secret WEFT_* system env; the WEFT_WH_*
      // secrets are re-read from process.env at retry time (RetryScheduler).
      if (isTransientFailure(result)) {
        if (result.attempts && result.attempts > 1) {
          const systemEnv = Object.fromEntries(
            Object.entries(env).filter(([k]) => !k.startsWith('WEFT_WH_')),
          );
          this.retryScheduler.enqueue(task.matcherId, task.action, systemEnv, redactUrl(result.url), result.error)
    ...
    if (results.length > 0 && this.options.onWebhookResults) {
      ...
      // SECURITY (audit B1): deliver REDACTED url copies — a host-app callback
      // consumer that logs/persists result.url would otherwise leak the expanded
      // webhook secret (Slack/Discord path tokens). The internal log/history/queue
      // paths already redact separately (above); this closes the callback path.
      // Shallow-copy each result so the internal `results` array is untouched.
      this.options.onWebhookResults(results.map((r) => ({ ...r, url: redactUrl(r.url) })));
    }
  }
