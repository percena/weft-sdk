...
import { existsSync, mkdirSync, chmodSync, readFileSync, writeFileSync, readdirSync, rmSync } from 'node:fs';
...
import { assertValidSlug, assertPathWithinRoot, isValidSlug } from './utils/slug.ts';
 ...
 * @throws Error if the slug is invalid or the resulting path escapes the sources directory
 */
export function getSourcePath(workspaceRootPath: string, sourceSlug: string): string {
  assertValidSlug(sourceSlug, 'source slug');
  const sourcesDir = getWorkspaceSourcesPath(workspaceRootPath);
  const sourcePath = join(sourcesDir, sourceSlug);
  assertPathWithinRoot(sourcesDir, sourcePath);
  return sourcePath;
}
...
export function ensureSourcesDir(workspaceRootPath: string): void {
  ...
  // SECURITY (audit hardening): re-assert 0o700 — mkdirSync's mode only applies
  // when the directory is created, so a pre-existing dir with a looser mode
  // (e.g. 0755) would otherwise stay world-traversable.
...
export function loadWorkspaceSources(workspaceRootPath: string): LoadedSource[] {
  ...
  for (const entry of entries) {
    // Skip directories whose names are not valid slugs so a single bad
    // folder can't break enumeration (loadSource throws on invalid slugs)
 ...
 * that don't have filesystem folders.
 */
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search the docs MCP for service-specific guidance)
 ...
 * @throws Error if the slug is invalid (path traversal protection)
 */
