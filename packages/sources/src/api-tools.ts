...
import { createSdkMcpServer, tool } from './sdk-server.ts';
import { z } from 'zod';
import type { ApiConfig } from './types.ts';
import { debug, redactHeaders, redactUrl } from './utils/debug.ts';
 ...
 *
 * SSRF hardening: the endpoint path is resolved against the base URL and must
 * stay on the base URL's origin. This blocks protocol-relative paths
 * ("//evil.com/x") and absolute-URL paths ("https://evil.com/x") from
 * redirecting authenticated requests to another host.
 *
...
export function buildUrl(
  ...
  const resolvedOrigin = new URL(path, baseUrl).origin;
  if (resolvedOrigin !== baseOrigin) {
    throw new Error(
      `API path ${JSON.stringify(path)} resolves to origin ${resolvedOrigin}, outside the source base URL origin ${baseOrigin}`
    );
  }
...
export function createApiTool(
  ...
  return tool(
    ...
    async (args: { path: string; method: string; params?: Record<string, unknown>; _intent?: string }) => {
      ...
      try {
        ...
        // SECURITY: redact the query-auth secret from the URL before
        // logging (query-auth embeds the API key as ?queryParam=…).
        const secretQueryParams = config.auth?.type === 'query' && config.auth.queryParam
          ? [config.auth.queryParam]
          : [];
        debug(`[api-tools] ${config.name}: ${method} ${redactUrl(url, secretQueryParams)}`);
        ...
        debug(`[api-tools] ${config.name}: headers=${JSON.stringify(redactHeaders(fetchOptions.headers as Record<string, string>))}, bodyLength=${fetchOptions.body ? String(fetchOptions.body).length : 0}`);

        // SSRF: block redirects to private/loopback/link-local targets (e.g. an
        // open redirect on the source API 302→169.254.169.254). blockPrivateInitial
        // is false because a source baseUrl may legitimately be an internal API the
        // operator configured; only public→private redirects are blocked. Mirrors
        // undici's default redirect-follow + method downgrade for the legit case.
        // resolveIps is injected so DNS names that resolve to private ranges are
        // caught (without it the guard checks only literal-IP hostnames).
