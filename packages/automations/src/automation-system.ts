...
import type { AutomationsConfig, AutomationEvent, AutomationMatcher, PendingPrompt, WebhookActionResult, AppEvent, AgentEvent, SdkAutomationCallbackMatcher, SdkAutomationInput } from './types.ts';
...
export class AutomationSystem implements AutomationsConfigProvider {
   ...
   * This is the backend-agnostic entry point for non-Claude backends (Codex, Copilot)
   ...
   */
  async executeAgentEvent(event: AgentEvent, input: SdkAutomationInput, _signal?: AbortSignal): Promise<number> {
    if (!this.config) return 0;
