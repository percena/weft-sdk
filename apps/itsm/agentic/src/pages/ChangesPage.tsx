 ...
 * the always-visible change row (the detail modal may not be open). Only
 * `actor === "agent"` events are replayed; the X-Weft-Actor override stamps
 * agent-driven tool calls with `actor = "agent"`. Non-change events return
 * null so they don't trigger a spurious replay+refresh on this tab.
 */
function toChangeActionEvent(event: ItsMEvent): WeftActionEvent | null {
  if (event.actor !== "agent") return null;
  if (!event.action.startsWith("change.")) return null;
  const change = event.data.change as { id?: string } | undefined;
  ...
  return {
    actor: "agent",
    action: event.action,
    target: `change-row:${change.id}`,
    label: `${describe} ${change.id}`,
  };
}
...
export default function ChangesPage() {
  ...
  const refresh = useCallback(() => {
    ...
    if (sel) {
      api.changes.get(sel.id).then(setSelected).catch(() => {});
    }
  }, [load]);
