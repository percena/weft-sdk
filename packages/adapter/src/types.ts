...
import type { AgentEvent, Workspace } from '@weft/core/types';
import type { FileAttachment, AuthRequest, LoadedSource, McpClientPool, SourceManager, AutomationSystem, RecoveryMessage, ModelProvider, LlmAuthType, LlmProviderType, AbortReason, SessionConfig as Session } from './stubs.ts';
import type { ThinkingLevel } from './thinking-levels.ts';
import type { PermissionMode } from './mode-manager.ts';

// Re-export types that consumers need
export { AbortReason } from './stubs.ts';
export type { RecoveryMessage, LlmAuthType, LlmProviderType } from './stubs.ts';
...
import type { BackendInitResult } from "./auth/provider-auth.ts";
 ...
 *
 * Extends BackendInitResult from the auth module, adding a warning
...
export interface BackendHostRuntimeContext {
  ...
  isPackaged: boolean;
  /** Optional runtime override for Node.js executable */
 ...
 * 2. Enable the facade pattern in the agent runtime
...
export interface AgentBackend {
   ...
   * - Backends with native steering (e.g., OpenAI) inject the message into the
   ...
   * - Claude: no-op (they don't use bridge-mcp-server)
   */
  ...
  onSpawnSession: ((request: import('./stubs.ts').SpawnSessionRequest) => Promise<import('./stubs.ts').SpawnSessionResult>) | null;
}
...
export interface BackendConfig extends CoreBackendConfig {
   ...
   * - 'anthropic' → Claude (via @weft/providers)
   * - 'openai' → Codex (via @weft/providers)
   * - 'copilot' → placeholder (uses anthropic driver until dedicated driver exists)
   */
