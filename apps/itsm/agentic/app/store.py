...
``"system"`` for seed/reset). The agentic adds the weft ``X-Weft-Actor: agent``
override (``weft.get_actor``) so agent-driven events are tagged ``"agent"``
(the flag the action-bridge replays as an automated live cursor).
"""
...
from .weft import get_actor as _weft_actor
...
class Store:
    ...
    def emit(self, actor: str, action: str, data: dict[str, Any]) -> Event:
        actor = _weft_actor() or actor  # X-Weft-Actor:agent → "agent" (action-bridge replay flag)
